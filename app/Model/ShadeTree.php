...
class ShadeTree extends Model
{
    use HasFactory;
    use SoftDeletes;
    protected $table = 'master_shade_tree';
    protected $primaryKey = 'id';
    protected $guarded = ['id'];

    public static function listByName($name){
        return Cache::remember("tree_shade|$name", config("constant.ttl"), function() use($name){
            return ShadeTree::when(!empty($name), function($builder) use($name){
